...
public class _1220_Count_Vowels_Permutation {
    ...
    public static void main(String[] args) {
        int count = countVowelPermutation(5);
...
//            dp[i][4] = (dp[i - 1][2] + dp[i - 1][3]) % MOD; // 'u' can follow {'i', 'o'}
            dp[i][0] = (dp[i - 1][1]) % MOD; // 'a' can follow {'e', 'i', 'u'}
            dp[i][1] = (dp[i - 1][0] + dp[i - 1][2]) % MOD; // 'e' can follow {'a', 'i'}
            dp[i][2] = (dp[i - 1][0] + dp[i - 1][1] + dp[i - 1][3] + dp[i - 1][4]) % MOD; // 'i' can follow {'e', 'o'}
            dp[i][3] = (dp[i - 1][2] + dp[i-1][4]) % MOD; // 'o' can follow {'i'}
            dp[i][4] = (dp[i - 1][0]) % MOD; // 'u' can follow {'i', 'o'}
        }
